...
public :
    ...
    Tetris(Point reference_pos = Point(1, 1), int key_esc = KEY_ESC, int key_right = KEY_RIGHT, int key_left = KEY_LEFT, int key_rotate = KEY_UP, int key_down = KEY_DOWN, int key_space=KEY_SPACE);	
    int Run(void);                             // 테트리스 실행 : running_이 true인 동안 RunStep을 반복적으로 실행, (추가) 끝날 때 최종 스코어를 반환
	...
	void CheckLevel(void); //레벨 체크해서 레벨마다 떨어지는 속도 조절
	int ReturnScore(void) { return score_.ReturnScore(); } //스코어 반환
...
private :
	...
	bool scoreon_;			//스코어가 화면 상에 출력 중인가
